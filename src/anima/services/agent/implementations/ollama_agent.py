...
class OllamaAgent(AgentInterface):
    ...
    def __init__(
        ...
        **kwargs
        ...
        logger.info(f"OllamaAgent 初始化完成: model={model}, base_url={self.base_url}")

    @classmethod
    def from_config(cls, config: "LLMBaseConfig", system_prompt: str = "", **kwargs) -> "OllamaAgent":
        """
        从配置对象创建实例
        ...
        if not isinstance(config, OllamaLLMConfig):
            raise TypeError(f"OllamaAgent 需要 OllamaLLMConfig，收到: {type(config)}")
