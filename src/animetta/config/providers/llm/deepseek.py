...
class DeepSeekLLMConfig(LLMBaseConfig):
    ...
    model: str = Field(
        default="deepseek-v4-flash", description="Model name: deepseek-v4-flash / deepseek-v4-pro"
    )
    base_url: str = Field(
        default="https://api.deepseek.com/v1", description="DeepSeek API Base URL"
    )
    temperature: float = Field(
        default=0.9, ge=0, le=2, description="Realtime roleplay sampling temperature"
    )
