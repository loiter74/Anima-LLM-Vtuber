"""
Provider engine lifetime and application-owned pool adapters.
...
class ProviderPool:
    """Application-owned facade around an isolated provider-pool state type."""

    def __init__(self, state_type: type[ServicePool] | None = None) -> None:
        self._state: type[ServicePool] = state_type or cast(
            type[ServicePool],
            type(
                "_ApplicationProviderPoolState",
                (ServicePool,),
                {
                    "_llm": None,
                    "_tts": None,
    ...
    async def init(
        ...
        await self._state.init(config, model_manager, observation_recorder)

    def configure_runtime(self, config: Any, model_manager: Any | None = None) -> None:
        self._state.configure_runtime(config, model_manager)

    def get_context(self) -> dict[str, Any]:
        return self._state.get_context()

    def apply_llm_config(self, llm_config: Any, system_prompt: str | None = None) -> None:
        self._state.apply_llm_config(llm_config, system_prompt)
    ...
    def get_readiness_snapshot(
        ...
        *,
        ...
        return self._state.get_readiness_snapshot(
            config=config,
            model_manager=model_manager,
            frontend=frontend,
        )
...
default_provider_pool = ProviderPool(ServicePool)
