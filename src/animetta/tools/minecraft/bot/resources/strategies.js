...
export function findNearestMatchingBlock(bot, mcData, blockNames, maxDistance) {
...
async function surfaceSweep(bot, ctx) {
  let found = findNearestMatchingBlock(bot, ctx.mcData, ctx.def.blocks, ctx.options.maxDistance);
  ...
  for (let ring = 1; ring <= 3 && _withinBudget(ctx); ring++) {
    ...
    found = findNearestMatchingBlock(bot, ctx.mcData, ctx.def.blocks, ctx.options.maxDistance);
    if (found) return _candidate(bot, found, 'surface_sweep');
...
async function safeDescent(bot, ctx) {
  ...
  const pos = getBotPosition(bot);
  if (!pos) return null;
  if (pos.y <= pref.max) return null; // 已在/低于目标层，无需下挖
  const maxDig = Math.min(16, pos.y - pref.max); // 受限深度
  for (let dy = 1; dy <= maxDig && _withinBudget(ctx); dy++) {
    ...
    const found = findNearestMatchingBlock(bot, ctx.mcData, ctx.def.blocks, ctx.options.maxDistance);
...
async function branchMine(bot, ctx) {
  ...
  for (let i = 1; i <= MAX_BRANCH && _withinBudget(ctx); i++) {
    ...
    const ahead = bot.blockAt(new Vec3(pos.x + i, pos.y, pos.z));
    if (!ahead || ahead.name === 'air') continue;
    if (ahead.name === 'lava' || ahead.name === 'flowing_lava') {
      throw locatorError('UNSAFE_AREA', {
        reason: 'lava ahead in branch',
        strategy: 'branch_mine',
      });
    }
    try {
