...
async def iron_pickaxe():
    proc = await asyncio.create_subprocess_exec(
        "node", "index.js", "localhost", "25565", "IronBot",
    ...
    sys.stdout.flush()
    ok = await retry(
        "collect", {"block_type": "oak_log", "count": 8},
        "oak_log", 8, tries=15, timeout=90, label="wood",
    )
    i = await inv()
    ...
    i = await inv()
    sys.stdout.write(f"Planks x4: {r['status']} | planks={i.get('oak_planks', 0)} log={i.get('oak_log', 0)}\n")
    sys.stdout.flush()
    ...
    sys.stdout.write(f"Table: {r['status']} | table={table_count} planks={i.get('oak_planks', 0)}\n")
    sys.stdout.flush()
        ...
        if isinstance(pos, dict):
            ...
            for dy in range(0, -5, -1):
                for dx, dz in [(0, 0), (1, 0), (-1, 0), (0, 1), (0, -1)]:
                    r = await send("place", {"block_type": "crafting_table", "x": x + dx, "y": y + dy, "z": z + dz})
    ...
    if table_pos:
        await send("goto", {"x": table_pos["x"], "y": table_pos["y"], "z": table_pos["z"]}, timeout=15)
    ...
    sys.stdout.flush()
    ok = await retry(
        "collect", {"block_type": "stone", "count": 6},
        "cobblestone", 3, tries=15, timeout=90, label="stone",
    )
    i = await inv()
    ...
    sys.stdout.flush()
    ok = await retry(
        "collect", {"block_type": "coal_ore", "count": 3},
        "coal", 1, tries=15, timeout=90, label="coal",
    )
    i = await inv()
    ...
    sys.stdout.flush()
    ok = await retry(
        "collect", {"block_type": "iron_ore", "count": 3},
        "raw_iron", 1, tries=15, timeout=120, label="iron",
    )
    i = await inv()
