...
运行：PYTHONPATH=src python -m animetta.tools.minecraft.other.scripts.run_voyager_skill
"""
...
async def main() -> int:
    ...
    logger.info("[run] 执行 skill code-body (eval_code, 最多 200s)...")
    ...
    if vr.passed:
        logger.success(
            f"[run] ★ GOAL MET ★ wooden_pickaxe={inv.get(GOAL_ITEM, 0)} | verify gate={vr.gate}"
        )
        goal_met = True
    else:
    ...
    logger.info(
        "[run] bot 保持在线，等待 LUN077 登录附身（MC 客户端连 localhost:25565）。保持 10 分钟..."
    )
