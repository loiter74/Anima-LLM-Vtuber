...
class DeepSeekLLM:
    def __init__(self):
        self._c = AsyncOpenAI(base_url="https://api.deepseek.com/v1", api_key=os.environ["DEEPSEEK_API_KEY"])

    async def chat(self, messages):
        r = await self._c.chat.completions.create(model="deepseek-chat", messages=messages, temperature=0, max_tokens=512)
        return type("R", (), {"content": r.choices[0].message.content})()


async def main():
    config = MinecraftConfig(
        enabled=True, mode=MinecraftMode.FALLBACK,
    ...
    async def run_code(code: str) -> dict:
        return await bridge.send_command("eval_code", {"code": code, "timeout": 60_000}, timeout=70.0)
    ...
    gen = await generate_with_iteration(
        ...
        ["has_iron_pickaxe >= 1"], run_code, llm, max_iters=4,
    )
    if gen.success:
    ...
    snapshot = {"inventory": inv, "position": rd.get("position"), "health": rd.get("health"), "food": rd.get("food")}
    vr = await verify("craft iron_pickaxe", ["has_iron_pickaxe >= 1"], snapshot, llm=None)
    if vr.passed:
        logger.success(f"★★★ GOAL ACHIEVED: iron_pickaxe={inv.get('iron_pickaxe', 0)} (verify gate={vr.gate}) ★★★")
    else:
