...
test('serves an existing static asset without rewriting its path', async () => {
  const assets = createAssets(new Map([
    ['/assets/app.js', new Response('app', { status: 200 })],
  ]))

  const response = await worker.fetch(
    new Request('https://animetta.example/assets/app.js'),
...
test('falls back to index.html for a client-side route', async () => {
  const assets = createAssets(new Map([
    ['/index.html', new Response('<main>Animetta</main>', {
  ...
  const response = await worker.fetch(
    new Request('https://animetta.example/settings'),
    { ASSETS: assets.binding },
  )
