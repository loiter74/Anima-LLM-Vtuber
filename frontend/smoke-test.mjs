...
if (releaseMode) {
  await context.addInitScript(() => {
    ...
    HTMLMediaElement.prototype.play = function (...args) {
      ...
      state.play_calls += 1;
      this.addEventListener('ended', () => { state.ended += 1; }, { once: true });
      this.addEventListener('error', () => { state.errors.push('media-error'); }, { once: true });
      try {
...
const coreUi = {
  passed: Boolean(title.trim()) && hasCanvas && bodyText.length >= 100 && buttons.length > 0 && inputs.length > 0,
  title,
...
if (releaseMode) {
  ...
  releaseAcceptance.provider_rows_exact = [
    ...
    'alice',
  ].every(value => ttsText.includes(value))
  ...
  await page.waitForFunction(
    value => {
      ...
      return Boolean(list?.textContent?.includes(value))
  ...
  const assistantText = await page.locator('[data-testid="message-list"] > div.flex').last().innerText();
  ...
  releaseAcceptance.chinese_turn_complete = messageText.includes(prompt)
    && /[\u4e00-\u9fff]/.test(assistantText)
    && releaseAcceptance.marker_leaks.length === 0;
  releaseAcceptance.passed = releaseAcceptance.provider_rows_exact
...
const failures = consoleErrors.length + pageErrors.length + requestFailures.length + httpErrors.length;
