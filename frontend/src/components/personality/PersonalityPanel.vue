...
watch(
  [() => store.availablePersonas, () => store.currentPersona],
...
<template>
  <div class="flex flex-col h-full">
    ...
    <div v-show="!collapsed" class="flex-1 overflow-y-auto px-5 py-4 space-y-4">
      <!-- Current mode badge -->
      <div>
        <label class="text-xs font-medium text-c-text-dim uppercase tracking-wider mb-2 block"
          >当前模式</label
        >
        <span
          class="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg text-xs font-medium"
          :class="
            store.currentMode === 'streaming'
      ...
      <div>
        ...
        <select
          ...
          class="w-full px-3 py-2 rounded-xl bg-c-bg/80 border border-c-border/40 text-sm text-c-text focus:outline-none focus:border-c-accent/50 transition-colors appearance-none cursor-pointer"
        >
          <option value="" disabled>选择人设...</option>
          <option v-for="p in store.availablePersonas" :key="p" :value="p">{{ p }}</option>
        </select>
        <button
          class="mt-2 w-full px-3 py-2 rounded-xl text-xs font-medium transition-all flex items-center justify-center gap-1.5"
          :class="
            store.personaSuccess
      ...
      <div>
        ...
        <div class="flex items-center justify-between bg-c-card/80 rounded-xl px-3 py-2.5">
          <span class="text-xs text-c-text">{{
            store.currentMode === 'streaming' ? '已开启' : '已关闭'
          }}</span>
          <button
            ...
            :class="[
              store.currentMode === 'streaming'
                ? 'bg-c-accent'
                : 'bg-c-bg/80 border border-c-border/40',
          ...
          >
            <span
              ...
              :class="
                store.currentMode === 'streaming' ? 'translate-x-[22px]' : 'translate-x-[2px]'
              "
            />
          </button>
        </div>
        ...
        <div class="bg-c-card/80 rounded-xl px-3 py-2.5">
          <div class="flex items-center justify-between mb-1.5">
            ...
            <span class="text-xs text-c-accent font-medium tabular-nums">{{
              store.memoryInfluence.toFixed(1)
            }}</span>
          </div>
          <input
            type="range"
      ...
      <div>
        ...
        <div class="bg-c-card/80 rounded-xl px-3 py-2.5 flex items-center gap-2">
          <span class="text-sm">💭</span>
          <span v-if="store.currentMood" class="text-sm text-c-text capitalize">{{
            store.currentMood
          }}</span>
      ...
      <div>
        <div class="flex items-center justify-between mb-2">
          <label class="text-xs font-medium text-c-text-dim uppercase tracking-wider"
            >MBTI 人格</label
          >
          <button
            class="w-7 h-7 flex items-center justify-center rounded-lg bg-c-bg/50 text-c-text-dim hover:text-c-text hover:bg-c-bg/70 transition-colors"
        ...
        <div v-show="!mbtiCollapsed" class="space-y-3">
          <!-- Type badge -->
          <div class="bg-c-card/80 rounded-xl px-3 py-2.5 flex items-center gap-2">
            <span v-if="computedMbtiType" class="text-sm font-bold text-c-accent">{{
              computedMbtiType
            }}</span>
            <span v-else class="text-sm text-c-text-muted">暂无检测数据</span>
          </div>
          ...
          <div class="bg-c-card/80 rounded-xl px-3 py-2.5 space-y-2.5">
            <div v-for="dim in dimensions" :key="dim.label" class="space-y-1">
              ...
              <div class="flex items-center gap-2">
                ...
                <span class="text-xs tabular-nums text-c-text-dim w-6 text-right shrink-0">{{
                  dim.val
                }}</span>
              </div>
            </div>
