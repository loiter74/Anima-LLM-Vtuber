...
<template>
  <div class="w-full">
    <!-- 梗卡片 -->
    <div
      class="p-5 rounded-2xl bg-c-card/50 border hover:bg-c-card/60 transition-all"
      :class="voteGlowClass || 'border-c-border'"
    >
      <div class="text-lg font-medium text-c-text leading-relaxed mb-3">「{{ meme.text }}」</div>
      <div class="flex flex-wrap gap-1.5 mb-3">
        <span class="px-2 py-0.5 text-xs rounded-full bg-c-primary/10 text-c-primary">{{
          meme.source_platform
        }}</span>
        <span
          v-for="tag in meme.tags.slice(0, 5)"
          :key="tag"
          class="px-2 py-0.5 text-xs rounded-full bg-c-bg text-c-text-secondary"
          >{{ tag }}</span
        >
      </div>
      <div class="grid grid-cols-3 gap-2 text-xs text-c-text-secondary">
        <div>
          <span class="block opacity-50">机制</span><span>{{ mechanismLabel }}</span>
        </div>
        <div>
          <span class="block opacity-50">情感</span><span>{{ toneLabel }}</span>
        </div>
        <div class="flex items-center gap-1.5">
          ...
          <svg class="w-4 h-4 shrink-0" viewBox="0 0 36 36">
            ...
            <path
              ...
              :stroke-dashoffset="100 - fitScore * 100"
      ...
      <a
        v-if="meme.cognitive_analysis?.source_url"
        :href="meme.cognitive_analysis.source_url"
        target="_blank"
        class="block mt-3 text-xs text-c-primary/60 hover:text-c-primary underline"
        >🔗 B站来源</a
      >
    </div>
    ...
    <Transition name="chat-msg">
      <div v-if="feedbackVisible && feedback" class="mt-3 flex items-start gap-2">
        <div
          class="w-7 h-7 rounded-full bg-c-primary/20 flex items-center justify-center text-sm shrink-0 mt-0.5"
        >
          🤖
        </div>
...
.chat-msg-leave-active {
  animation: msgIn 0.25s var(--ease-out-expo) reverse;
}
@keyframes msgIn {
  from {
    opacity: 0;
