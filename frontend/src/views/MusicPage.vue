...
function handleMixedPlay() {
  isMixedPlaying.value = true
}
function handleMixedPause() {
  isMixedPlaying.value = false
}
function handleMixedEnded() {
  isMixedPlaying.value = false
}
...
<template>
  <div class="flex-1 h-full overflow-hidden">
    ...
    <div class="h-full overflow-y-auto">
      ...
      <div v-if="isIdle" class="min-h-full flex items-center justify-center p-6">
        <div class="w-full max-w-xl flex flex-col items-center gap-6">
          <!-- Title -->
          <div class="text-center space-y-2">
            <h1 class="text-2xl font-bold text-c-text">🎵 音乐制作</h1>
            <p class="text-sm text-c-text-dim">输入B站视频链接，AI自动完成人声替换</p>
          </div>
          ...
          <div class="w-full glass p-6 space-y-4">
            <div class="flex gap-3">
              <input
                ...
                class="flex-1 px-4 py-2.5 rounded-xl bg-c-bg/40 border border-c-border/30 text-sm text-c-text placeholder-c-text-dim/50 outline-none focus:border-c-accent/50 transition-all"
          ...
          <!-- Recent works -->
          <div v-if="recentItems.length > 0 || loadingRecent" class="w-full space-y-3">
            <div class="text-xs text-c-text-dim font-medium tracking-wide">最近作品</div>

            <div v-if="loadingRecent" class="text-xs text-c-text-dim animate-pulse px-1">
              加载中...
            </div>
            ...
            <div
              ...
              :key="item.session_id"
              class="flex items-center gap-3 px-3 py-2.5 rounded-xl bg-c-bg/40 border border-c-border/30 hover:bg-c-bg/60 hover:border-c-border/50 transition-all"
            >
              <button
                class="w-8 h-8 flex items-center justify-center rounded-full bg-c-accent/20 text-c-accent hover:bg-c-accent/40 hover:scale-105 transition-all shrink-0"
              ...
              <a
                :href="item.audio_url"
                class="px-2.5 py-1 rounded-lg bg-c-accent/20 text-c-accent text-xs hover:bg-c-accent/30 transition-all shrink-0"
                target="_blank"
              >
                DL
              </a>
              ...
              <a
                ...
                class="px-2.5 py-1 rounded-lg bg-c-bg/40 text-c-text-dim text-xs hover:text-c-text hover:bg-c-bg/60 transition-all shrink-0"
                target="_blank"
              >
                Sub
              </a>
      ...
      <div v-else class="p-6 space-y-6">
        ...
        <div v-if="isProcessing" class="max-w-xl mx-auto space-y-4">
          <div class="flex items-center justify-between">
            <div class="flex items-center gap-2">
              ...
              <span class="text-xs text-c-text-dim bg-c-bg/40 px-2 py-0.5 rounded-full font-mono">
                {{ Math.round(store.progress) }}%
              </span>
            </div>
            <button
              class="px-3 py-1.5 rounded-lg bg-c-error/10 text-c-error text-xs hover:bg-c-error/20 transition-all"
              @click="cancel"
            >
              取消
            </button>
          ...
          <ProcessTimeline :current-stage="store.status" :progress="store.progress" />
        ...
        >
          ...
          <button
            class="mt-2 px-3 py-1 rounded-lg bg-c-bg/40 text-c-text-dim text-xs hover:text-c-text transition-all"
            @click="handleBack"
          >
            返回重试
          </button>
        ...
        <div v-if="hasResult && store.result" class="space-y-6">
          ...
          <div class="flex items-center gap-3">
            <button
              class="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-c-bg/40 border border-c-border/30 text-xs text-c-text-dim hover:text-c-accent hover:border-c-accent/30 transition-all"
              @click="handleBack"
            >
              <svg
                width="14"
            ...
            <div v-if="store.result.video_title" class="text-xs text-c-text-dim truncate flex-1">
          ...
          <div class="grid grid-cols-1 lg:grid-cols-3 gap-4">
            ...
            <div class="glass p-4 space-y-3 flex flex-col">
              ...
              <div class="flex-1">
                ...
                />
                <div v-else class="text-xs text-c-text-dim py-4 text-center">不可用</div>
              </div>
              <a
                v-if="store.result.original_url"
                :href="store.result.original_url"
                target="_blank"
                class="inline-flex items-center gap-1.5 text-xs text-c-text-dim hover:text-c-accent transition-colors"
              >
                <svg
                  width="12"
                  height="12"
            ...
            <!-- 🎵 Replaced + mixed -->
            <div class="glass p-4 space-y-3 flex flex-col ring-1 ring-c-border-accent">
              <div class="flex items-center gap-2">
                <span class="text-lg">🎵</span>
                <div>
              ...
              <a
                ...
                target="_blank"
                class="inline-flex items-center gap-1.5 text-xs text-c-accent hover:text-c-accent-hover transition-colors"
              >
                <svg
                  width="12"
                  height="12"
            ...
            <div class="glass p-4 space-y-3 flex flex-col">
              ...
              <a
                ...
                target="_blank"
                class="inline-flex items-center gap-1.5 text-xs text-c-text-dim hover:text-c-text transition-colors"
              >
                <svg
                  width="12"
                  height="12"
