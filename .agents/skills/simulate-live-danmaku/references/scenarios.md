...
| `hourly` | 181 | 一小时陪跑：每分钟 1 条保底弹幕，并按合成低/中/高热度追加 0/1/2 条 |
...
- 要求每分钟固定发送、持续一小时并随热度变化：`hourly`、`--speed 1`。它包含 60 条保底、60 条热度增量和 60 次合成热度快照，最后在 3600 秒发送结束事件；热度是可复现测试数据，不是真实房间指标。
