...
| 场景 | 事件 | 用途 |
|---|---:|---|
| `smoke` | 4 | 默认快速冒烟：一次真实回复，附带入场、点赞与关注传输 |
| `daily` | 7 | 多轮日常聊天：问候、追问、话题互动与非回复事件 |
| `quiet` | 5 | 冷场与长间隔：验证等待后的回复、页面状态与节奏 |
| `crowd` | 10 | 短时高峰：连续弹幕、点赞与关注；会产生多次真实 Provider 调用 |
| `support` | 7 | 支持互动：普通弹幕、礼物与醒目留言文本、点赞及后续追问 |

事件时间为相对 `offset_ms`。重放链路会等待每个可回复事件完成后再处理下一条，因此耗时主要由可回复事件数和 LLM/TTS 决定；`speed` 只缩放事件间隔。`crowd` 用于链路积压和恢复观察，不代表精准并发压测。

## 选择建议

- “发一条、播下、测下实时弹幕”或不清楚选什么：`smoke`。
- 验证多轮回复、上下文延续或连续播放：`daily`。
