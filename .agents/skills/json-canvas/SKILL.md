...
---
# JSON Canvas Skill
This skill enables Codex to create and edit valid JSON Canvas files (`.canvas`) used in Obsidian and other applications.
...
**症状**: 在 Excalidraw 根目录下创建已存在的项目目录（如 `Excalidraw/MC-Bot/` 和 `Excalidraw/Animetta/MC-Bot/`）。

**原因**: 没有先检查现有的目录结构。

**正确做法**:
1. 先 `read` 目标父目录，查看已有的子目录
2. 如果项目目录已存在，直接在其中创建文件
3. 遵循 AGENTS.md 的分层原则：大项目 → 子决策

### 错误 2: 节点尺寸太小

**症状**: 节点宽度 < 200px，高度 < 60px，导致：
- 文字频繁换行
- 显示拥挤，难以阅读
- 与其他节点比例失调

**原因**: 没有参考 Recommended Sizes 表格。

**正确做法**:
- 小文本节点: 250-300px 宽，80-100px 高
- 中等文本节点: 300-400px 宽，120-180px 高
- 分组节点: 包含所有子节点 + 25-50px padding

### 错误 3: 编号冲突

**症状**: 新建的 canvas 文件与已有文件编号重复（如两个 `01-xxx.canvas`）。

**原因**: 没有先检查目录下已有的文件。

**正确做法**:
1. 先 `read` 目录，列出所有 `.canvas` 文件
2. 找到最大编号，新文件从下一个编号开始
3. 如果是补充设计，可以用 02, 03, 04... 继续编号
...
**正确做法**:
- 节点之间至少 50px 间距
- 分组内节点至少 20px padding
- 使用对齐工具保持整洁

### 错误 5: 颜色使用不一致

**症状**: 随意使用颜色，没有遵循预设颜色含义。
