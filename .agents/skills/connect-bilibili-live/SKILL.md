...
description: 连接、切换、查询或断开 Animetta 后端持有的唯一真实 Bilibili 直播弹幕会话，并在正式 /live.html 验收弹幕、LLM 回复与 TTS 实际播放。用户要求连接 B 站直播、启动弹幕姬、把默认直播间接到 Animetta、切换直播间或检查真实弹幕链路时使用。
---
...
6. 连接成功必须同时满足：
   - CLI 返回 `ok=true`、`state=live`、目标 `room_id`、`elapsed_ms<60000`；
   - 页面 `#socketStatus[data-state=connected]`；
   - 页面 `#livestreamStatus[data-state=live]`；
   - 从页面热启动计时起到以上断言全部成立小于 60 秒。
7. 保持直播会话运行，不在验收后自动断开。脚本退出只关闭本地控制连接。
8. 用户要求完整链路验收时加载 `$qa-testing-playwright`，等待第一条真实可回复弹幕并按同一 ID 链路断言：
...
9. 等待窗口内没有真实可回复弹幕时报告 `awaiting_real_danmaku`。不得注入合成弹幕后宣称真实链路通过。
