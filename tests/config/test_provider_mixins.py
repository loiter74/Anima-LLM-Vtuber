...
class TestMixinComposition:
    ...
    def test_llm_base_has_all_mixin_fields(self):
        from animetta.core.config.providers.llm.base import LLMBaseConfig
    ...
    def test_asr_base_has_api_and_model(self):
        from animetta.core.config.providers.asr.base import ASRBaseConfig
    ...
    def test_tts_base_has_api_and_model(self):
        from animetta.core.config.providers.tts.base import TTSBaseConfig
    ...
    def test_vc_base_has_device(self):
        from animetta.core.config.providers.vc.base import VCBaseConfig
        fields = VCBaseConfig.model_fields
        assert "device" in fields
        assert "type" in fields

    def test_separation_base_has_device(self):
        from animetta.core.config.providers.separation.base import SeparationBaseConfig
        fields = SeparationBaseConfig.model_fields
        assert "device" in fields
        assert "type" in fields

    def test_vad_base_has_type(self):
        from animetta.core.config.providers.vad.base import VADBaseConfig
        fields = VADBaseConfig.model_fields
        assert "type" in fields
        assert "sample_rate" in fields

    def test_deepseek_inherits_all_from_mixins(self):
        from animetta.core.config.providers.llm.deepseek import DeepSeekLLMConfig
