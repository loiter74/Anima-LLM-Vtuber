...
class TestConversation:
    ...
    async def test_pipeline(self, server):
        ...
        await sio.emit("chat:text", {"text": "Hello!", "user_id": "t", "from_name": "T"})
        ...
        any(
            isinstance(d, dict) and d.get("index", -1) >= 0
            for d in ev.get("chat:live2d_action", [])
        )
        errs = ev.get("system:error", [])
        expr_em = ev["chat:expression"][0].get("emotion", "") if ev.get("chat:expression") else ""
        mot_idx = (
            ev["chat:live2d_action"][0].get("index", -1) if ev.get("chat:live2d_action") else -1
        )
        print(
            f"Events: {sorted(ev.keys())} | sentence={has_text} emotion={expr_em} motion={mot_idx} errors={errs}"
        )
