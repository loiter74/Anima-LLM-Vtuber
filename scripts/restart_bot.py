...
async def main():
    from animetta.tools.minecraft.core.bridge import get_bridge, MinecraftBridge
    ...
    for label, params, timeout in steps:
        ...
        r = await new_bridge.send_command('craft' if 'craft' in label else 'collect', params, timeout=timeout)
    ...
    r = await new_bridge.send_command('craft', {'recipe': 'stone_pickaxe', 'count': 1}, timeout=30.0)
    print(f'    → {r.get("status")}: {r.get("result", "")}', flush=True)

    # Set goal
    await new_bridge.send_command('setgoal', {'goal': 'mine diamonds y<16, survive 2 nights'}, timeout=5.0)
    ...
    if s.get('status') == 'success':
        ...
        tools = {k: v for k, v in inv.items() if any(w in k for w in ['pickaxe', 'sword', 'axe', 'planks', 'log'])}
        print(f'[status] Pos=({pos.get("x",0):.0f},{pos.get("y",0):.0f},{pos.get("z",0):.0f}) HP={s["result"]["health"]} Food={s["result"]["food"]}', flush=True)
        print(f'[status] Tools: {tools}', flush=True)

    print('[restart] Bootstrap done. Autonomous loop active.', flush=True)
    print('[restart] Survival rules: weapon recovery | combat eating | health flee | pickaxe equip | lava escape', flush=True)
    ...
    try:
        while new_bridge.is_running:
            ...
            if st.get('status') == 'success':
                ...
                pick = 'iron' if inv.get('iron_pickaxe',0) > 0 else 'stone' if inv.get('stone_pickaxe',0) > 0 else 'wood' if inv.get('wooden_pickaxe',0) > 0 else 'none'
                print(f'  [{pos.get("x",0):.0f},{pos.get("y",0):.0f},{pos.get("z",0):.0f}] HP={s.get("health")} Food={s.get("food")} Dia={dia} Fe={iron} Pick={pick}', flush=True)
